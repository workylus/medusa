...
An API Route is a REST endpoint that exposes commerce features to external applications, such as the admin dashboard, storefronts, or third-party systems.
...
The API route you'll create accepts the brand's name in the request body. So, you'll first create a validation schema for the parameters expected in the request body.
...
You export a validation schema that expects in the request body an object having a `name` property whose value is a string. You'll use this schema when creating the API route next.
...
`MedusaRequest` accepts the request body's type as a type argument. You use a type inferred from the Zod schema you created in the earlier step.
